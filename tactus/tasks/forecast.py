...
class Forecast(PySurfexBaseTask):
    ...
    def sst_sic_input(self, intp_bddir):
        ...
        while current_datetime <= end_datetime:
            ...
            self.fmanager.input(
                f"{intp_bddir}/{source}", source, register="system.intp_bddir"
            )
    ...
    def execute(self):
        ...
        self.fmanager.input(
            initfile, f"ICMSH{self.cnmexp}INIT", register="system.intp_bddir"
        )
        if not self.surfex:
            initfile_sfx = None
        ...
        if initfile_sfx is not None:
            self.fmanager.input(
                initfile_sfx,
                f"ICMSH{self.cnmexp}INIT.sfx",
                register="system.intp_bddir_sfx",
            )
        ...
        while current_datetime <= end_datetime:
            ...
            self.fmanager.input(
                f"{intp_bddir}/{source}", target, register="system.intp_bddir"
            )
            current_datetime += self.bdint
            i += 1
